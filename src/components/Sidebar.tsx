...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
    ...
    const sidebarFooterText = useMemo(() => {
        if (!configs?.sidebarBusinessInfo?.enabled || !configs?.sidebarBusinessInfo?.footerText) {
            return null;
        }
        const currentYear = new Date().getFullYear();
        return configs.sidebarBusinessInfo.footerText.replace('{year}', currentYear.toString());
    }, [configs]);
            ...
            >
                ...
                <div className="flex-shrink-0 flex items-center justify-between p-4 border-b h-16 bg-primary-700 border-primary-600 md:hidden">
                    <a href="/#" className="flex items-center space-x-3 text-white">
                        ...
                        <div>
                            <span className="block self-center text-lg font-semibold whitespace-nowrap leading-tight">{configs.app.title}</span>
                             {configs.app.subtitle && (
                                <span className="block text-xs text-primary-200/80 leading-tight">{configs.app.subtitle}</span>
                            )}
                        </div>
                    </a>
                ...
                <div className="flex-grow flex flex-col overflow-hidden">
                    ...
                    {configs.sidebarBusinessInfo.enabled && (
                        <div className="flex-shrink-0 p-4 border-t border-gray-200 dark:border-gray-700 flex flex-col gap-3 bg-gray-50 dark:bg-gray-700/50">
                            {(configs.sidebarBusinessInfo.logo || configs.sidebarBusinessInfo.title) && (
                                <a 
                                    href={configs.sidebarBusinessInfo.link} 
                                ...
                                >
                                    {configs.sidebarBusinessInfo.logo && <SidebarLogo className="h-8 w-8" />}
                                    {configs.sidebarBusinessInfo.title && (
